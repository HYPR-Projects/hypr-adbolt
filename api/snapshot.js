...
async function runSnapshot({ url, creativeUrl, creativeSize, creativeKind, freeze, proxies, publicOrigin }) {
  ...
  try {
    ...
    const bake = await page.evaluate(bakeCreativeInPage, creativeDataUri, creativeSize || '', liveMeta);
    ...
    return {
      html: finalHtml,
      slots: bake,
      meta: { engine, consentHandled, durationMs: Date.now() - started, title, live: !!liveMeta },
    };
  } catch (err) {
